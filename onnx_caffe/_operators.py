...
def _convert_pool(node, graph, err):
    ...
    ## IF WE DONT HAVE round_mode=1 option, we need to limit the paramters to following range
    # max_stride = 1
    # if strides[0] > max_stride or strides[1] > max_stride:
    #    return err.unsupported_op_configuration(node, "Unsupported pool stride, must <= 1, got {}".format(strides))
    # if pads[0] > 1 or pads[1] > 1:
    #    return err.unsupported_op_configuration(node, "Unsupported pool pads , must <= 1, got {}".format(pads))
